...
#define MAX_N_WALKS 1000000
i32 walks[MAX_N_WALKS][MAX_N_NODES] = {};
i32 walk_lengths[MAX_N_WALKS] = {};
b8 have_visited_small_node_twice[MAX_N_WALKS] = {};
i32 n_walks = 0;
...
void walk(i32 walk_id, i32 node_id) {
  ...
  for (i32 i = 0; i < n_nodes; i++) {
    if (adjmat[node_id][i]) {
      bool should_set_double_small_node_visit = false;
      if (is_node_small[i] && does_walk_have_node(walk_id, i)) {
        if (
          have_visited_small_node_twice[walk_id] ||
          i == start_id ||
          i == end_id
        ) {
          continue;
        } else {
          should_set_double_small_node_visit = true;
        }
      }
      ...
      have_visited_small_node_twice[new_walk_id] = have_visited_small_node_twice[walk_id] ||
        should_set_double_small_node_visit;
      walk(new_walk_id, i);
    }
  }
}
